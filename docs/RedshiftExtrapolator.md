...
> **历史文档说明（2026-09-10）：** 本文的导入路径、距离术语及部分模型适用范围已经过时。
> 当前红移转换的独立推导、计数／曝光约定和源码审计请见
> [LF 红移转换：推导核验与实现规格](lf_redshift_transform.md)。
> 新代码从 `jinwu.lf` 导入相关能力；新文档中的未来接口尚未实现。
> 本文共动距离形式的幂律缩放在平直宇宙学、默认普通幂律归一化下与新文档等价，
> 但不能据此推广到任意宇宙学、红移模型或仪器模拟。
