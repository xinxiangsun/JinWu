...
class XspecConfig:
	...
	xspec_cosmo: str = XSPEC_COSMO_PLANCK18
...
def generate_redshift_lightcurves(
	...
	dt: float,  # 时间分辨率（秒）
	mean_rate_z0: float,  # [deprecated] unused; retained for backward compat only — formula uses rate_z directly
	K_z0: float,  # [deprecated] unused; retained for backward compat only — formula uses rate_z directly
	# 目标红移
	z: float,
	# 背景后验（用于采样背景率）
	...
	2. cfg.params 已由调用方通过 _adjust_params_for_redshift(z) 更新
	   （包含红移参数、norm 缩放）。XspecKFactory 直接获取
	   目标红移的 rate_z [cts/s] — 不再单独计算 K(z)。
	3. 红移缩放：时间 × (1+z)/(1+z0)，保持轮廓形状
	4. 计算源信号期望计数率：
	   rate(z) = counts_norm × rate_z
	   这里：
	   - counts_norm = net_rate(z0) / rate_z0（无量纲归一化轮廓）
	   - rate_z = XSPEC 在目标红移 z 处的绝对计数率 [cts/s]（已折叠 K⋅flux）
	...
	# 1. 通过 XspecKFactory 获取目标红移的 K, rate, flux
	...
	# 源信号期望计数率：
	# counts_norm = net_rate(t) / mean_rate_z0（无量纲归一化轮廓）
	# rate_z = XSPEC 在红移 z 处计算的总计数率 [cts/s]
	# 归一化轮廓 × 目标红移处的计数率 → 期望计数率 [cts/s]
	source_rate_expected = counts_norm * rate_z
