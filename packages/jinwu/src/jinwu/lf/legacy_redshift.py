"""Legacy redshift extrapolation helper for LF detectability work.

This module contains the older XSPEC-stateful ``RedshiftExtrapolator`` that
previously lived in ``jinwu.core.utils``.  Prefer ``jinwu.lf.redshift`` for new
Monte-Carlo light-curve based extrapolation, but keep this implementation here
for existing workflows that rely on the grid/SNR helper API.
"""
...
except ModuleNotFoundError:  # pragma: no cover - depends on HEASoft install
    xs = None
    FakeitSettings = None
    AllData = None
...
except ModuleNotFoundError:  # pragma: no cover - depends on optional XSPEC stack
    XspecKFactory = None
